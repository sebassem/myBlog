...
One of the most common methods of onboarding servers to Azure Arc is using a short-lived service principal with least privilege (using the **Azure Connected Machine Onboarding** role), yet there can be some concerns around having this service principal's identity compromised specially if it's not recycled frequently, where an attacker can use it to onboard other servers outside of your organization.
